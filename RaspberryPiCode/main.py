...
def detection_process(queue : mp.Queue, detection_queue : mp.Queue, model_path : str):
    yolo_detection = YOLODetection(model_path)
    while True:
        if not queue.empty():
            frame, timestamp = queue.get()
            rect = yolo_detection.detect(frame)
            if rect is None:
                continue
...
def main():
    ...
    # Assuming 'best 1.pt' is in the root or accessible relative path. 
    # Adjusted to point to the file location based on previous ls.
    # The file 'best 1.pt' was seen in the root of the project.
    ...
    try:
        ...
        # Fallback just in case, or we should ask user. 
        # Using the relative path assuming running from RaspberryPiCode dir.
        estimator_data = pd.read_csv('Data/2024-Note/FullData.csv')

    processes = [
        mp.Process(target=frame_capture_process, args=(frame_queue, camera_id, resolution, camera_fps)),
        mp.Process(target=detection_process, args=(frame_queue, detection_queue, model_path)),
